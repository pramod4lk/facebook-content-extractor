...
def test_unknown_media_has_no_status(tmp_path: Path) -> None:
    ...
    assert manifest.get_status(media_type="photo", media_id="1") is None
...
def test_insert_and_lookup(tmp_path: Path) -> None:
    ...
    assert manifest.get_status(media_type="photo", media_id="1") == DownloadStatus.DOWNLOADED
...
def test_records_are_scoped_per_media_type(tmp_path: Path) -> None:
    manifest = make_manifest(tmp_path)
    ...
    assert manifest.get_status(media_type="reel", media_id="1") is None
...
def test_persists_across_manifest_instances(tmp_path: Path) -> None:
    ...
    assert reopened.get_status(media_type="photo", media_id="1") == DownloadStatus.DOWNLOADED
